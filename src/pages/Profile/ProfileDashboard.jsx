...
export default function ProfileDashboard() {
    ...
    // Support deep-linking via location.state.section (from Header dropdown)
    const [activeSection, setActiveSection] = useState(
        location.state?.section || 'tickets'
    );
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                const userData = await profileService.getUser(userId);
    ...
    // Update section if navigating from another page with state
    useEffect(() => {
    ...
    const renderContent = () => {
        if (activeSection === 'tickets') {
            ...
            return <MyInfo user={user} onUserUpdate={handleUserUpdate} />;
            ...
            return (
                <div className="p-6 bg-white rounded-[20px] shadow-sm border border-zinc-100 min-h-[500px] flex items-center justify-center font-medium text-zinc-500">
